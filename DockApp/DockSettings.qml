...
Singleton {
    ...
    // (e.g. the ML4W editor script when the dock runs on its own), falls back to
    // xdg-open — the default application for JSON files. The command is passed
    // as an argument rather than spliced into the script, so quotes in it
    // cannot break the fallback logic.
    function editConfig(): void {
        Quickshell.execDetached(["bash", "-c",
            'f="$HOME/.config/ml4w-dock/config.json"; c="${1/#\\~/$HOME}";'
